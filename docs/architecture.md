...
classes. `adapters` is the only layer allowed to expose spglib, spinspg, or amcheck
calling conventions. `symmetry` classifies operations and computes exact periodic site
mappings. `magnetism` assigns moments and evaluates explicit preliminary checks.
